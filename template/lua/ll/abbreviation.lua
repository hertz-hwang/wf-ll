...
function this.func(key_event, env)
  ...
  --if incoming == 'M' and length == 1 then -- 重复一字词
  if incoming == 'M' then
  ...
  elseif lookup[incoming] ~= nil then -- 重复并插入
    ...
    env.engine:commit_text(selection.text)
  elseif incoming == 'E' or incoming == 'R' then -- 重复词的首字或末字
    if incoming == 'E' then
  ...
  elseif incoming == 'O' and length == 2 then -- 叠词重复二字词
    ...
    context:commit()
    env.engine:commit_text(snow.sub(selection.text, -1, -1))
  elseif incoming == 'J' then -- Ａ着Ａ着
    context:confirm_current_selection()
    context:commit()
    env.engine:commit_text('着' .. selection.text .. '着')
  elseif incoming == 'V' then -- Ａ来Ａ去
    context:confirm_current_selection()
    context:commit()
